...
(define eval-lambda
  (lambda (list env)
    (append (cons 'closure (cdr list)) env)))
...
(define apply-closure
  (lambda (list)
    (evaluate (car(cdr(cdr(car list)))) (append-env (car(cdr(car list))) (cdr(cdr(cdr(car list)))) (cdr list)))
    ))
...
(define apply-function
  (lambda (list)
    (cond ((procedure? (car list)) (apply (car list) (cdr list)))
          ...
          ((eqv? (car list) 'closure) list)
          ((eqv? (car(car list)) 'closure) (apply-closure list))
